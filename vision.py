...
class Vision:
    ...
    def vision(self,acquisition_delay,white_threshold,plot,P):
        '''
        General init function (init for the grid creation)
    ...
    def get_frame_from_aruco(self,frame,plot=False):
        ...
        PLUS: calculate the cm/pixel ratio AND replace the start/goal Arucos by white squares to avoid confusion during grid creation
        '''
        ...
        aruco_margin_factor = 0.5 #How much size of the Aruco do we want to add ?
        for c, i in zip(corners_after, ids_after):
            if i in [1, 3]:
                pts = c[0]
                center = c[0].mean(axis=0)
    ...
    def overlay_grid_on_cropped(self, frame=None, show=True, line_color=(0,255,0), start_color=(0,255,0), goal_color=(255,0,0)):
        ...
        try:
            if self.thymio_pos is not None:
    ...
    def get_grid(self,grid_Nx,grid_Ny,frame,white_th,P,plot=False):
        ...
        Advice: do not use the default case with 0,0 and P not defined because the A* will be very long to compute

        P: it creates a square grid with cell size P (in pixels)
        '''
        height, width, _ = frame.shape

        #Default + squared case -> Nx and Ny overwrited
